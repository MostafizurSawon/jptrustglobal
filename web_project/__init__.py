...
class TemplateLayout:
    def init(self, context):
        ...
        context['user'] = getattr(self, 'request', None) and getattr(self.request, 'user', None)

        layout = context["layout"]
        context.update({
            "layout_path": TemplateHelper.set_layout(f"layout_{layout}.html", context),
            "rtl_mode": True if self.request.COOKIES.get('django_text_direction') == "rtl"
                        else settings.TEMPLATE_CONFIG.get("rtl_mode"),
        })
